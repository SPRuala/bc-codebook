export default{
	dbName: 'code-book-db',
	dbPath: '_doc/code-book-db',
	//判断数据库是否打开
	isOpen(){
		//数据库打开了返回true、否则返回 false
	...
	// 创建 CREATE TABLE IF NOT EXISTS 、 dbTable 是表名，不能用数字开头、括号里是表格的字段名
	...
	//模糊查询
	selectTableDataMH(dbTable, uname, namevalue, upass, passvalue, urrn, rrnvalue) {
	    if (dbTable !== undefined) {
	        if (uname !== undefined && upass !== undefined && urrn !== undefined) {
	            var sql = `SELECT * FROM ${dbTable} WHERE ${uname} LIKE '%${namevalue}%' AND ${upass} LIKE '%${passvalue}%' AND ${urrn} LIKE '%${rrnvalue}%'`;
	        }
	        if (uname !== undefined && upass !== undefined && urrn == undefined) {
	            var sql = `SELECT * FROM ${dbTable} WHERE ${uname} LIKE '%${namevalue}%' AND ${upass} LIKE '%${passvalue}%'`;
	        }
	        if (uname !== undefined && upass == undefined && urrn == undefined) {
	            var sql = `SELECT * FROM ${dbTable} WHERE ${uname} LIKE '%${namevalue}%'`;
	        }
	        if (uname == undefined) {
	            var sql = `SELECT * FROM ${dbTable}`;
	        }
